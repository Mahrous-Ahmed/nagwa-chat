...
public class Send_English_Text extends Test_Base {
...
String EnglishMessage=ReadProperties.userDate.getProperty("EnglishText");
    ...
    @Description("user send English text")
    @Parameters({"Timezone"})
    public void send_English_text(String Timezone)  {
        chatGroupScreenObject=new Chat_Group_Screen(driver);
        chatGroupScreenObject.typingText(EnglishMessage);
        chatGroupScreenObject.sendMessage();
        driver.manage().timeouts().implicitlyWait(1, TimeUnit.SECONDS);
        Assert.assertEquals(chatGroupScreenObject.TextOfLastMessage(),EnglishMessage);
